...
export class ProfileComponent implements OnInit {
  ...
  addPost(){
    this.router.navigateByUrl('/ref-add');
  }
  userManager(){
  ...
  populateForms(pd: Refractory) {       // метод обновляет данные во временном обьекте (refractoryService.Refractory) типа Refractory на основании обьекта выделенного из списка pd в представлении

    this.postService.refractory = Object.assign({}, pd);   // Object.assign - предотвращает корректировку полей в  Object.assign({}, pd);
    this.router.navigate(['/user-ref']);
  }
